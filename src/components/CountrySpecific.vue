<template>
  <div class="row q-mt-md">
    ...
    <q-input
      ...
      class="input-large col-sm col-xs-12 q-ml-md-lg q-ml-xs-none"
    />
    <!-- Region input field -->
    <q-input
...
export default {
  ...
  methods: {
    ...
    getProvinceObject(abbr) {
      let province = this.optionForProvinceSelector.filter(v => v.abbr.indexOf(abbr) > -1)
    ...
    filterFn(val, update) {
      update(() => {
        if (val === '') {
          ...
          this.filterOptions = this.optionForProvinceSelector.filter(v => v.name.toLowerCase().indexOf(needle) > -1)
        }
      })
    }
  },
